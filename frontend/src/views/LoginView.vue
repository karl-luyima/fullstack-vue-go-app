...
async function handleSubmit() {
  try {
    ...
    // auth.error already holds the message — nothing else to do here.
  }
}
</script>
...
<template>
  <div class="min-h-screen flex items-center justify-center bg-slate-100">
    <div class="w-full max-w-sm rounded-xl bg-white p-8 shadow-sm">
      <h1 class="mb-6 text-xl font-semibold text-slate-900">Log in</h1>
      ...
      <p class="mt-4 text-center text-sm text-slate-500">
        No account?
        <router-link to="/register" class="font-medium text-blue-600">Register</router-link>
      </p>
    </div>
  </div>
</template>